...
mongo_users_collection = "api_users"  # the collection for the user model
...
async def total_docs_in_db(conn: AsyncIOMotorClient) -> int:
    ...
    """
    return await conn[mongo_db][mongo_users_collection].count_documents({})
...
async def get_all(conn: AsyncIOMotorClient) -> dict:
    ...
    docs = []
    async for doc in conn[mongo_db][mongo_users_collection].find():
        docs.append(BaseUserInDB(id=str(doc['_id']), **doc).dict())
    if docs:
        return {'total': total_docs, "data": docs}
...
async def get_user_by_email(
    ...
    """
    row = await conn[mongo_db][mongo_users_collection].find_one({"email": email})
    if row:
        return BaseUserInDB(id=str(row['_id']), **row)
...
async def create_api_user(
    ...
    row = await conn[mongo_db][mongo_users_collection].insert_one(api_user.dict())
...
async def update_api_user(
    ...
    api_user.updated_at = datetime.utcnow()
    updated_at = await conn[mongo_db][mongo_users_collection].update_one(
...
async def update_api_user_password(conn: AsyncIOMotorClient, user: BaseUserCreate) -> BaseUserInDB:
    api_user = await get_user_by_email(conn, user.email)
    data: dict = {
        "updated_at": datetime.utcnow(),
        "password": user.password
    }
    await conn[mongo_db][mongo_users_collection].update_one(
        {"email": api_user.email}, {'$set': data}
    )
    return api_user
